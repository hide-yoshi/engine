...
contract ACE is IAZTEC, Ownable, NoteRegistryManager {
    ...
    function mint(
        ...
        require(
            address(registry.behaviour) != address(0x0),
        ...
        require(
            category == uint8(ProofCategory.MINT),
            "this is not a mint proof"
        );

        bytes memory _proofOutputs = this.validateProof(
            _proof,
            _proofSender,
            _proofData
        );
    ...
    function burn(
        ...
        require(
            category == uint8(ProofCategory.BURN),
    ...
    function validateProof(
        ...
        assembly {
            ...
            mstore(
                add(memPtr, 0x64),
                sload(add(0x01, commonReferenceString_slot))
            )
            mstore(
                add(memPtr, 0x84),
                sload(add(0x02, commonReferenceString_slot))
            )
            mstore(
                add(memPtr, 0xa4),
                sload(add(0x03, commonReferenceString_slot))
            )
            mstore(
                add(memPtr, 0xc4),
            ...
            mstore(
                add(memPtr, 0xe4),
                sload(add(0x05, commonReferenceString_slot))
            )
        ...
        if (((_proof >> 8) & 0xff) == uint8(ProofCategory.BALANCED)) {
            ...
            for (uint256 i = 0; i < length; i += 1) {
                ...
                bytes32 validatedProofHash = keccak256(
                    abi.encode(proofHash, _proof, msg.sender)
                );
     ...
     */
    function clearProofByHashes(uint24 _proof, bytes32[] calldata _proofHashes)
        external
    {
     ...
     */
    function setCommonReferenceString(bytes32[6] memory _commonReferenceString)
        public
    {
        require(
            isOwner(),
    ...
    function invalidateProof(uint24 _proof) public {
        ...
        require(
            validators[epoch][category][id] != address(0x0),
     ...
     */
    function getCommonReferenceString()
        public
        view
     ...
     */
    function getValidatorAddress(uint24 _proof)
        public
        view
        returns (address validatorAddress)
    {
        ...
        if (queryInvalid) {
            require(
                validatorAddress != address(0x0),
                "expected the validator address to exist"
            );
            require(
                isValidatorDisabled == false,
